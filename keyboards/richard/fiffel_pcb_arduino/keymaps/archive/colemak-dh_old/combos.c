...
// layer 1
const uint16_t PROGMEM layer1[] = {KC_S, KC_T, KC_N, KC_E, COMBO_END};
...
const uint16_t PROGMEM osl_layer4_right[] = {KC_N, KC_E, KC_I, KC_O, COMBO_END}; // NEIO
const uint16_t PROGMEM osl_layer4_left[] = {KC_A, KC_R, KC_S, KC_T, COMBO_END};  // ARST
...
combo_t key_combos[] = {
    ...
    // when held, it's shift, when pressed it is RETURN
    COMBO(left_shift, MT(MOD_LSFT, KC_ENT)),
    /* COMBO(right_shift, KC_RSFT), */
    // when held, it's shift, when pressed it is RETURN
    COMBO(right_shift, MT(MOD_RSFT, KC_ENT)),
